    ...
    <section className=" lg:px-28 md:p-20 sm:p-12 p-7 bg-back text-center relative slider1 flex flex-col items-center">
     <div className="w-full text-center flex flex-col items-center !gap-y-4 mb-8 md:mb-16">
                  {testimonialData?.title1 && <h4 className='text-green3 bg-[#E1F1D8] px-7 mb-3 font-bold text-sm rounded-lg py-3 w-max'>{testimonialData?.title1}</h4>}
                    {testimonialData?.title2 && <h2 className='text-zinc-800 text-3xl md:text-5xl font-bold'>{testimonialData?.title2}</h2>}
      ...
      >
       ...
       >
         <div className=" w-full  h-full overflow-hidden  group  ">
           <h3 className="text-xl font-bold mb-3 text-green1">{cards?.heading}</h3>
           <p className="text-zinc-900 text-lg leading-[1.4] pb-3 border-b border-zinc-500">{cards.text}</p>
...
<div className="flex gap-2 w-max items-center mt-3">
      ...
      <div className={`${uniqueId}-next max-sm:!hidden swiper-button-next !top-[63%] after:text-green2 lg:!right-20 after:font-bold`}></div>

<div className={`${uniqueId}-prev max-sm:!hidden swiper-button-prev !top-[63%] after:text-green2 lg:!left-20 after:font-bold `}></div>
