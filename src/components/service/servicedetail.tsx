  ...
  return (
    <div className="mb-6 md:p-24 relative p-3 flex flex-wrap justify-between items-start max-md:gap-7">
      <div className="md:w-[66%] shadow-lg w-full p-5 text-zinc-800 md:space-y-12 space-y-4 max-md:mt-5">
        ...
        <p className="text-lg font-semibold text-zinc-700">{data.para1}</p>
        <Image src={data.img1} alt="service" className="w-full h-full" />
        <p className="text-lg font-semibold text-zinc-700">{data.para2}</p>

        <h3 className="md:text-4xl text-2xl font-bold">{data.title2}</h3>
        <p className="text-lg font-semibold text-zinc-700">{data.para3}</p>
        <p className="text-lg font-semibold text-zinc-700">{data.para4}</p>

        <p className="text-lg font-semibold text-zinc-700">{data.para5}</p>

        <div className=" w-full max-md:mt-5 h-full relative p-8 max-lg:mt-10">
          <Image src={data.img2} alt="map" className="w-full mt-4" />
        </div>
        <p className="text-lg font-semibold text-zinc-700">{data.para6}</p>
        <p className="text-lg font-semibold text-zinc-700">{data.para7}</p>
      </div>

      <div className="md:sticky top-8 md:w-[30%] w-full">
        <div className=" p-6 w-full bg-white shadow-lg">
          <h3 className="mb-3 font-bold text-xl md:text-2xl text-green2">
            Send Us Message
          </h3>
          ...
          <form action="contact.php" method="POST" className=" w-full mt-6">
            <div className="w-full flex justify-between flex-wrap gap-y-5">
              ...
              <button
                type="submit"
                className="rounded-lg mb-3 -mt-1 text-white font-bold text-base py-4 px-6 w-max bg-green2 hover:bg-black duration-300"
              >
                Submit Now &nbsp;→
              </button>
            </div>
